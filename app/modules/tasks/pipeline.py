...
def launch_pipeline(session_id: str, ctx: DiscoveryContext):
    ...
    opts_katana = KatanaContext(session_id=session_id).to_json()

    discovery_chord = chord(
        group(
            task_subfinder.s(session_id, ctx_json),
            task_katana.s(session_id, ctx_json, opts_katana),
        ...
        build_discovery_context.s(session_id, ctx_json)  # fires when all 6 finish
    )
